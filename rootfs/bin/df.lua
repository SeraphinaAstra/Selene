...
if not info then
    print("df: " .. tostring(err or "not mounted"))
    return
end

local used = info.block_count - (info.free_blocks or 0)
local pct = info.block_count > 0 and math.floor(used / info.block_count * 100) or 0

local total_kb = math.floor(info.block_count * info.block_size / 1024)
local used_kb  = math.floor(used * info.block_size / 1024)
local free_kb  = math.floor((info.free_blocks or 0) * info.block_size / 1024)
